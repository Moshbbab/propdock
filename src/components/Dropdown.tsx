...
>(({ className, children, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.SubTrigger
    ...
    className={cx(
      ...
      // disabled
      "data-[disabled]:pointer-events-none data-[disabled]:text-warm-grey-2 data-[disabled]:hover:bg-none dark:data-[disabled]:text-warm-grey-1",
      // focus
      "focus-visible:bg-warm-white/80 data-[state=open]:bg-warm-white/80 focus-visible:dark:bg-warm-grey/60 data-[state=open]:dark:bg-warm-grey/60",
      // hover
      "hover:bg-warm-white/80 hover:dark:bg-warm-grey/60",
      //
      className,
    )}
    {...props}
  >
    {children}
    <RiArrowRightSLine
      className="ml-auto size-4 shrink-0 text-warm-grey-2 dark:text-warm-grey-1"
      aria-hidden="true"
    />
  </DropdownMenuPrimitives.SubTrigger>
))
...
>(({ className, collisionPadding = 8, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.Portal>
    <DropdownMenuPrimitives.SubContent
      ...
      className={cx(
        // base
        "relative z-50 overflow-hidden rounded-md border p-1 shadow-xl shadow-warm-grey/10",
        // widths
        "min-w-32",
        // heights
...
>(({ className, shortcut, hint, children, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.Item
    ...
    className={cx(
      ...
      "data-[disabled]:pointer-events-none data-[disabled]:text-warm-grey-2 data-[disabled]:hover:bg-none dark:data-[disabled]:text-warm-grey-1",
      // focus
      "focus-visible:bg-warm-white/80 focus-visible:dark:bg-warm-grey/60",
      // hover
      "hover:bg-warm-white/80 hover:dark:bg-warm-grey/60",
      className,
    )}
  ...
  >
    ...
    {hint && (
      <span
        className={cx(
          "ml-auto pl-2 text-sm text-warm-grey-2 dark:text-warm-grey-1",
        )}
      >
        {hint}
      </span>
  ...
  (
    ...
    >
      ...
      {hint && (
        <span
          className={cx(
            "ml-auto text-sm font-normal text-warm-grey-2 dark:text-warm-grey-1",
          )}
        >
          {hint}
        </span>
      ...
      {shortcut && (
        <span
          className={cx(
            "ml-auto text-sm font-normal tracking-widest text-warm-grey-2 dark:border-warm-grey-1/20 dark:text-warm-grey-1",
          )}
        >
          {shortcut}
        </span>
    ...
    >
      {iconType === "radio" ? (
        <span className="absolute left-2 flex size-4 items-center justify-center">
          <RiRadioButtonFill
            aria-hidden="true"
            className="size-full shrink-0 text-amber-600 group-data-[state=checked]/DropdownMenuRadioItem:flex group-data-[state=unchecked]/DropdownMenuRadioItem:hidden dark:text-amber-500"
          />
          <RiCheckboxBlankCircleLine
            aria-hidden="true"
            className="size-full shrink-0 text-warm-grey-2/50 group-data-[state=unchecked]/DropdownMenuRadioItem:flex group-data-[state=checked]/DropdownMenuRadioItem:hidden dark:text-warm-grey-1/50"
          />
        </span>
      ) : iconType === "check" ? (
        <span className="absolute left-2 flex size-4 items-center justify-center">
          <RiCheckLine
            aria-hidden="true"
            className="size-full shrink-0 text-warm-grey group-data-[state=checked]/DropdownMenuRadioItem:flex group-data-[state=unchecked]/DropdownMenuRadioItem:hidden dark:text-warm-white"
          />
        </span>
      ) : null}
      {children}
...
>(({ className, ...props }, forwardedRef) => (
  <DropdownMenuPrimitives.Separator
    ref={forwardedRef}
    className={cx(
      "-mx-1 my-1 h-px border-t border-warm-grey-2/20 dark:border-warm-grey-1/20",
      className,
    )}
    {...props}
  />
...
const DropdownMenuIconWrapper = ({
  ...
  return (
    <div
      className={cx(
        // text color
        "text-warm-grey dark:text-warm-grey-1",
        // disabled
        "group-data-[disabled]/DropdownMenuItem:text-warm-grey-2 group-data-[disabled]/DropdownMenuItem:dark:text-warm-grey-1/50",
        className,
      )}
      {...props}
    />
